...
def dictionary_out2file(R_dic, log_fileout):
    ...
    if R['laplace_opt'] == 'p_laplace2multi_scale' or R['laplace_opt'] == 'p_laplace2multi_scale':
...
def solve_Multiscale_PDE(R):
    ...
    if R['model'] == 'PDE_DNN_Cos_C_Sin_Base':
        W2NN, B2NN = DNN_base.initialize_NN_random_normal2_CS(input_dim, out_dim, hidden_layers, flag)
    else:
    ...
    with tf.device('/gpu:%s' % (R['gpuNo'])):
        with tf.variable_scope('vscope', reuse=tf.AUTO_REUSE):
            ...
            # 供选择的网络模式
            if R['model'] == 'PDE_DNN':
                U_NN = DNN_base.PDE_DNN(XYZST_it, W2NN, B2NN, hidden_layers, activate_name=act_func)
                U00_NN = DNN_base.PDE_DNN(XYZST00, W2NN, B2NN, hidden_layers, activate_name=act_func)
                U01_NN = DNN_base.PDE_DNN(XYZST01, W2NN, B2NN, hidden_layers, activate_name=act_func)
                U10_NN = DNN_base.PDE_DNN(XYZST10, W2NN, B2NN, hidden_layers, activate_name=act_func)
                U11_NN = DNN_base.PDE_DNN(XYZST11, W2NN, B2NN, hidden_layers, activate_name=act_func)
                U20_NN = DNN_base.PDE_DNN(XYZST20, W2NN, B2NN, hidden_layers, activate_name=act_func)
                U21_NN = DNN_base.PDE_DNN(XYZST21, W2NN, B2NN, hidden_layers, activate_name=act_func)
                U30_NN = DNN_base.PDE_DNN(XYZST30, W2NN, B2NN, hidden_layers, activate_name=act_func)
                U31_NN = DNN_base.PDE_DNN(XYZST31, W2NN, B2NN, hidden_layers, activate_name=act_func)
                U40_NN = DNN_base.PDE_DNN(XYZST40, W2NN, B2NN, hidden_layers, activate_name=act_func)
                U41_NN = DNN_base.PDE_DNN(XYZST41, W2NN, B2NN, hidden_layers, activate_name=act_func)
            elif R['model'] == 'PDE_DNN_scale':
                freqs = R['freqs']
                U_NN = DNN_base.PDE_DNN_scale(XYZST_it, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                U00_NN = DNN_base.PDE_DNN_scale(XYZST00, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                U01_NN = DNN_base.PDE_DNN_scale(XYZST01, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                U10_NN = DNN_base.PDE_DNN_scale(XYZST10, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                U11_NN = DNN_base.PDE_DNN_scale(XYZST11, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                U20_NN = DNN_base.PDE_DNN_scale(XYZST20, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                U21_NN = DNN_base.PDE_DNN_scale(XYZST21, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                U30_NN = DNN_base.PDE_DNN_scale(XYZST30, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                U31_NN = DNN_base.PDE_DNN_scale(XYZST31, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                U40_NN = DNN_base.PDE_DNN_scale(XYZST40, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                U41_NN = DNN_base.PDE_DNN_scale(XYZST41, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
            elif R['model'] == 'PDE_DNN_adapt_scale':
                freqs = R['freqs']
                U_NN = DNN_base.PDE_DNN_adapt_scale(XYZST_it, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                U00_NN = DNN_base.PDE_DNN_adapt_scale(XYZST00, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                U01_NN = DNN_base.PDE_DNN_adapt_scale(XYZST01, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                U10_NN = DNN_base.PDE_DNN_adapt_scale(XYZST10, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                U11_NN = DNN_base.PDE_DNN_adapt_scale(XYZST11, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                U20_NN = DNN_base.PDE_DNN_adapt_scale(XYZST20, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                U21_NN = DNN_base.PDE_DNN_adapt_scale(XYZST21, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                U30_NN = DNN_base.PDE_DNN_adapt_scale(XYZST30, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                U31_NN = DNN_base.PDE_DNN_adapt_scale(XYZST31, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                U40_NN = DNN_base.PDE_DNN_adapt_scale(XYZST40, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                U41_NN = DNN_base.PDE_DNN_adapt_scale(XYZST41, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
            elif R['model'] == 'PDE_DNN_FourierBase':
                freqs = R['freqs']
                U_NN = DNN_base.PDE_DNN_FourierBase(XYZST_it, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                U00_NN = DNN_base.PDE_DNN_FourierBase(XYZST00, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                U01_NN = DNN_base.PDE_DNN_FourierBase(XYZST01, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                U10_NN = DNN_base.PDE_DNN_FourierBase(XYZST10, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                U11_NN = DNN_base.PDE_DNN_FourierBase(XYZST11, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                U20_NN = DNN_base.PDE_DNN_FourierBase(XYZST20, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                U21_NN = DNN_base.PDE_DNN_FourierBase(XYZST21, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                U30_NN = DNN_base.PDE_DNN_FourierBase(XYZST30, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                U31_NN = DNN_base.PDE_DNN_FourierBase(XYZST31, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                U40_NN = DNN_base.PDE_DNN_FourierBase(XYZST40, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                U41_NN = DNN_base.PDE_DNN_FourierBase(XYZST41, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
            elif R['model'] == 'PDE_DNN_Cos_C_Sin_Base':
                freqs = R['freqs']
                U_NN = DNN_base.PDE_DNN_Cos_C_Sin_Base(XYZST_it, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                U00_NN = DNN_base.PDE_DNN_Cos_C_Sin_Base(XYZST00, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                U01_NN = DNN_base.PDE_DNN_Cos_C_Sin_Base(XYZST01, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                U10_NN = DNN_base.PDE_DNN_Cos_C_Sin_Base(XYZST10, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                U11_NN = DNN_base.PDE_DNN_Cos_C_Sin_Base(XYZST11, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                U20_NN = DNN_base.PDE_DNN_Cos_C_Sin_Base(XYZST20, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                U21_NN = DNN_base.PDE_DNN_Cos_C_Sin_Base(XYZST21, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                U30_NN = DNN_base.PDE_DNN_Cos_C_Sin_Base(XYZST30, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                U31_NN = DNN_base.PDE_DNN_Cos_C_Sin_Base(XYZST31, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                U40_NN = DNN_base.PDE_DNN_Cos_C_Sin_Base(XYZST40, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                U41_NN = DNN_base.PDE_DNN_Cos_C_Sin_Base(XYZST41, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
            ...
            if R['variational_loss'] == 1:
                dU_NN = tf.gradients(U_NN, XYZST_it)[0]      # * 行 2 列
                dU_NN_2norm = tf.reshape(tf.sqrt(tf.reduce_sum(tf.square(dU_NN), axis=-1)), shape=[-1, 1])  # 按行求和
                if R['laplace_opt'] == 'general_laplace':
                    laplace_pow = tf.square(dU_NN_2norm)
                    loss_it_variational = (1.0 / 2) *laplace_pow - tf.multiply(f(X_it, Y_it, Z_it, S_it, T_it), U_NN)
                else:
                    a_eps = A_eps(X_it, Y_it, Z_it, S_it, T_it)                          # * 行 1 列
                    laplace_p_pow = a_eps*tf.pow(dU_NN_2norm, p)
                    loss_it_variational = (1.0 / p) * laplace_p_pow - tf.multiply(f(X_it, Y_it, Z_it, S_it, T_it), U_NN)
    ...
    with tf.Session(config=config) as sess:
        ...
        for i_epoch in range(R['max_epoch'] + 1):
            ...
            if i_epoch % 1000 == 0:
                ...
                train_option = False
                if R['laplace_opt'] == 'general_laplace' or R['laplace_opt'] == 'p_laplace2multi_scale':
                    u_true2test, u_nn2test, lossIt2uprect = sess.run(
                        [U_true, U_NN, loss_it], feed_dict={XYZST_it: test_xyzst_bach, train_opt: train_option})
                else:
                    u_true2test = u_true
                    u_nn2test, lossIt2uprect = sess.run(
                        [U_NN, loss_it],  feed_dict={XYZST_it: test_xyzst_bach, train_opt: train_option})
...
if __name__ == "__main__":
    ...
    # &&&&&&&&&&&&&&&&&&&&&& 隐藏层的层数和每层神经元数目 &&&&&&&&&&&&&&&&&&&&&&&&&&&&
    if R['model'] == 'PDE_DNN_Cos_C_Sin_Base':
        R['hidden_layers'] = (250, 400, 400, 300, 300, 200)
    elif  R['model'] == 'PDE_DNN_FourierBase':
