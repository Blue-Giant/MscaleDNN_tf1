...
def solve_Multiscale_PDE(R):
    ...
    elif R['laplace_opt'] == 'p_laplace2explicit':
        if R['equa_name'] == 'multi_scale2D_7':
            region_lb = 0.0
            region_rt = 1.0
            mesh_number = 6
            f = MS_Laplace_eqs.force_side2E7(input_dim, out_dim)                       # f是一个向量
            u_true = MS_Laplace_eqs.true_solution2E7(input_dim, out_dim, eps=epsilon)
            u_left, u_right, u_bottom, u_top = MS_Laplace_eqs.boundary2E7(input_dim, out_dim, region_lb, region_rt,
                                                                          eps=epsilon)
            # A_eps要作用在u的每一个网格点值，所以A_eps在每一个网格点都要求值，和u类似
            A_eps = MS_Laplace_eqs.elliptic_coef2E7(input_dim, out_dim, eps=epsilon)

    # 初始化权重和和偏置的模式
    if R['weight_biases_model'] == 'general_model':
        flag1 = 'WB'
        if R['model'] == 'PDE_DNN_Cos_C_Sin_Base':
            W2NN, B2NN = DNN_base.initialize_NN_random_normal2_CS(input_dim, out_dim, hidden_layers, flag1)
        else:
    ...
    with tf.device('/gpu:%s' % (R['gpuNo'])):
        with tf.variable_scope('vscope', reuse=tf.AUTO_REUSE):
            ...
            if R['model'] == 'PDE_DNN':
                ...
                freqs = R['freqs']
                U_NN = DNN_base.PDE_DNN_Cos_C_Sin_Base(XY_it, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                ULeft_NN = DNN_base.PDE_DNN_Cos_C_Sin_Base(XY_left_bd, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                URight_NN = DNN_base.PDE_DNN_Cos_C_Sin_Base(XY_right_bd, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                UBottom_NN = DNN_base.PDE_DNN_Cos_C_Sin_Base(XY_bottom_bd, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
                UTop_NN = DNN_base.PDE_DNN_Cos_C_Sin_Base(XY_top_bd, W2NN, B2NN, hidden_layers, freqs, activate_name=act_func)
            ...
            elif R['variational_loss'] == 0:
                dUNNxy = tf.gradients(U_NN, XY_it)[0]  # * 行 2 列
                if R['laplace_opt'] == 'general_laplace':
                    dUNNx = tf.gather(dUNNxy, [0], axis=-1)
                    dUNNy = tf.gather(dUNNxy, [1], axis=-1)
                    ...
                    loss_it1 = tf.add(dUNNxx, dUNNyy) + f(X_it, Y_it)
                    square_loss_it = tf.square(loss_it1)
                else:
                    a_eps = A_eps(X_it, Y_it)
                    # dAxy = tf.gradients(a_eps, XY_it)[0]
                    # dAx = tf.gather(dAxy, [0], axis=-1)
                    # dAy = tf.gather(dAxy, [1], axis=-1)
                    #
                    # dUNNxy_norm = tf.reshape(tf.sqrt(tf.reduce_sum(tf.square(dUNNxy), axis=-1)), shape=[-1, 1])  # 按行求和
                    # dUNNx = tf.gather(dUNNxy, [0], axis=-1)
                    # dUNNy = tf.gather(dUNNxy, [1], axis=-1)
                    # UNNx_dUNNnorm = tf.multiply(dUNNx, tf.pow(dUNNxy_norm, p-2))
                    # UNNy_dUNNnorm = tf.multiply(dUNNy, tf.pow(dUNNxy_norm, p - 2))
                    # dUxdUnorm = tf.gradients(UNNx_dUNNnorm, XY_it)[0]
                    # dUUnorm_x = tf.gather(dUxdUnorm, [0], axis=-1)
                    #
                    # dUydUnorm = tf.gradients(UNNy_dUNNnorm, XY_it)[0]
                    # dUUnorm_y = tf.gather(dUydUnorm, [1], axis=-1)
                    #
                    # loss_it1 = tf.multiply(dAx, UNNx_dUNNnorm) + tf.multiply(a_eps, dUUnorm_x) + \
                    #                       tf.multiply(dAy, UNNy_dUNNnorm) + tf.multiply(a_eps, dUUnorm_y) + f(X_it, Y_it)
                    ...
                    if p == 2:
                        ...
                        dUxdUnorm = tf.multiply(tf.pow(dUNNxy_norm, p - 2), dUNNx)
                        dUydUnorm = tf.multiply(tf.pow(dUNNxy_norm, p - 2), dUNNy)
                    if R['equa_name'] == 'multi_scale2D_1':
                        AdUxdUnorm = dUxdUnorm
                        AdUydUnorm = dUydUnorm
                    else:
                    ...
                    square_loss_it = tf.square(-div_AdUdUnorm - f(X_it, Y_it))
            ...
            if R['laplace_opt'] == 'general_laplace' or R['laplace_opt'] == 'p_laplace2explicit':
                # 训练上的真解值和训练结果的误差
...
if __name__ == "__main__":
    ...
    # R['laplace_opt'] = 'p_laplace2explicit'
    ...
    # R['variational_loss'] = 0  # PDE变分

    R['optimizer_name'] = 'Adam'                          # 优化器
    R['learning_rate'] = 2e-4                             # 学习率
    R['learning_rate_decay'] = 5e-5
    # 学习率 decay
    R['train_group'] = 0                                  # 训练 loss 的模式
    ...
    # &&&&&&&&&&&&&&&&&&&&&& 隐藏层的层数和每层神经元数目 &&&&&&&&&&&&&&&&&&&&&&&&&&&&
    if R['model'] == 'PDE_DNN_Cos_C_Sin_Base':
        R['hidden_layers'] = (125, 200, 200, 100, 100, 50)  # 2*125+250*200+200*200+200*100+100*100+100*50+50*1=125300
    elif R['model'] == 'PDE_DNN_FourierBase':
        R['hidden_layers'] = (250, 200, 200, 100, 100, 50)  # 1*250+250*200+200*200+200*100+100*100+100*50+50*1=125300
    else:
